...
# relationship() = Python/ORM-level navigation of that connection
...
def get_a_task(id:int,session:Session=Depends(get_db), current_user: User = Depends(get_current_user)):
    query= session.query(Task).filter(Task.id==id,Task.owner_id==current_user.id)  # creating a query object 
...
def add_task(data:TaskCreateDTO,session:Session=Depends(get_db), current_user: User = Depends(get_current_user)):
...
def update_task(id:int,data:TaskUpdateDTO,session:Session=Depends(get_db),current_user: User = Depends(get_current_user)):
    task=session.query(Task).filter(Task.id==id,Task.owner_id==current_user.id).first()
    if task is None:
        raise HTTPException(
            status_code= 404,
            detail="Task Not Found"
...
def delete_a_task(id:int,session:Session=Depends(get_db),current_user: User = Depends(get_current_user)):
    task=session.query(Task).filter(Task.id==id,Task.owner_id==current_user.id).first()
    if task is None:
        raise HTTPException(
            status_code=404,
